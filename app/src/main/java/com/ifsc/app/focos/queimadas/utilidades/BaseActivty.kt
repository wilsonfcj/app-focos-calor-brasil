...
abstract class BaseActivty : AppCompatActivity(), MapElement {
    ...
    fun setColor() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            mToolbar?.navigationIcon
                ?.setColorFilter(getColor(R.color.icons), PorterDuff.Mode.SRC_ATOP)
        }
    }
    ...
    fun applyTint(icon: Drawable) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            icon.colorFilter = PorterDuffColorFilter(getColor(R.color.icons), PorterDuff.Mode.SRC_IN)
        } else {
            icon.colorFilter = PorterDuffColorFilter(
                resources.getColor(R.color.icons),
